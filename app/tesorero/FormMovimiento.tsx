...
export default function FormMovimiento({ alumnos }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (esCuota && !alumnoId) { setError('Selecciona el alumno para registrar una cuota'); return }
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {/* Alumno opcional para todos los tipos excepto gasto */}
      {tipo === 'ingreso' && (
        <div style={{ display: 'grid', gridTemplateColumns: esCuota ? '1fr 1fr' : '1fr', gap: '0.75rem', marginBottom: '0.75rem' }}>
          <div>
          ...
          {esCuota && (
            <div>
              ...
              <select value={mesCuota} onChange={e => setMesCuota(e.target.value)}>
                ...
                {MESES_2026.map(m => <option key={m.key} value={m.key}>{m.label} 2026</option>)}
              </select>
            </div>
          )}
        </div>
      ...
      {ok && <p style={{ color: '#16a34a', fontSize: '0.85rem', margin: '0 0 0.75rem', fontWeight: 600 }}>✓ Movimiento registrado correctamente</p>}
